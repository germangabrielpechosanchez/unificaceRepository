...
@auth.requires(auth.has_membership(role='admin')|auth.has_membership(role='gestion_retrofacturation_projects')|auth.has_membership(role='gestion_retrofacturation_edit')|auth.has_membership(role='gestion_retrofacturation_edit_add'))
def importation():
    #db(db.ci_payment.completed==datetime.date(3000,1,1)).update(to_update=False,)
    #read data from Octopus
    #"""
    import subprocess 
    script=r'C:\Users\T2-PEGE8301.06CEMTL\Desktop\environmentTestClonedFromGitHub\unificaceAdmin\web2py\applications\init\scripts\gestion_retrofacturation\read_octopus_retrofacturation_projects.py'
    p1 = subprocess.Popen([r'C:\Users\T2-PEGE8301.06CEMTL\Desktop\UnificaceTestEnvironnment\officialVersionOfUnificace\Python27\python.exe',script],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
def exportation():       
    ...
    total=0
    k_code={'xxx':'1001','':'1001','100171':'1001','100172':'1002','100175':'1005','100132':'1002','100174':'1004'}
    for r in rs:
        #purchase_price=r.purchase_price
        purchase_price=round(r.purchase_price*1.056879,2)
        r_descriptions=r.descriptions if r.descriptions else ''
        r_grm_code='' if r.grm_code==None else r.grm_code
        total-=purchase_price
        account_department=db(db.account_department.id==r.account).select(db.account_department.account,db.account_department.account_type).first()
        account_code=account_department.account
        code_ecriture=r.code_ecriture
        try:this_k_code=k_code[account_department.account_type]
        except:this_k_code='pas de code'
        this_l_code=this_o_code=this_p_code=''
        if account_department.account_type=='100132':
            this_l_code=account_code[:12]
            this_p_code=account_code[-2:]
            this_o_code=account_code[11:].replace(this_p_code,'')
            account_code=''
            code_ecriture=''
    ...
    #delete the file
    os.remove(file_name)
    #for test to return to before state
    #db(db.ci_payment.exported==today).update(exported=datetime.date(3000,1,1),to_update=False)#for test
...
def exportation_octopus():
    ...
    if rs:
        ...
        file_name=r'C:\Users\T2-PEGE8301.06CEMTL\Desktop\UnificaceTestEnvironnment\officialVersionOfUnificace\web2py\applications\init\scripts\gestion_retrofacturation\octopus\ci_csv.csv'
        ...
        ########################
        file_xml="ci_csv.xml"
        script=r'C:\Users\T2-PEGE8301.06CEMTL\Desktop\UnificaceTestEnvironnment\officialVersionOfUnificace\web2py\applications\init\scripts\gestion_retrofacturation\octopus\data_importer_csv.bat'
        #crete bat file
        this_file=open(script, 'w')
        bat_line=r"C:\Users\T2-PEGE8301.06CEMTL\AppData\Local\Octopus\ESI.Octopus.DataImporterApp.exe /Login:octsys01 /Password:Bonjour02 /ConfigFilePath:C:\Users\T2-PEGE8301.06CEMTL\Desktop\environmentTestClonedFromGitHub\unificaceAdmin\web2py\applications\init\scripts\gestion_retrofacturation\octopus\\"+file_xml+" /team:1 /LogFilePath:"
        log_file=r"C:\Users\T2-PEGE8301.06CEMTL\Desktop\environmentTestClonedFromGitHub\unificaceAdmin\web2py\applications\init\scripts\gestion_retrofacturation\octopus\DataImporter"+str(session.auth.user.id)+".log"
        bat_line+=log_file
        ...
        log_path=r"C:\Users\T2-PEGE8301.06CEMTL\Desktop\environmentTestClonedFromGitHub\unificaceAdmin\web2py\applications\init\scripts\gestion_retrofacturation\octopus"
