...
def index():
    ...
    return dict(grid="")
@auth.requires(auth.has_membership(role='admin')|auth.has_membership(role='gestion_fichiers_script_hl7'))
def folder_hl7_ok():
    response.menu+=[
        (T("Creation des fichiers vide de type .ok"), False, URL( f='folder_hl7_ok'),[]), 
        ]
...
def tables():
    #rs=db(db.mysql_files.id>0).select()
    #for r in rs:
    #    db(db.mysql_files.id==r.id).update(links=r.id)
    menu_url=[]
    ...
    if request.args(0):
        ...
        grid = SQLFORM.smartgrid(db[table],args=request.args[:1],maxtextlength=150,deletable=False,editable=False,create=False)
...
def load_file():
    ...
    if request.args:
        import os
        folder="\\\\s03vwdv00004.cemtl.rtss.qc.ca\\files_from_table\\myfiles"
        unique_file_name=db(db.mysql_files.id==int(request.args(0))).select(db.mysql_files.unique_file_name).first().unique_file_name.decode('UTF-8').encode('ISO-8859-1')
        path=os.path.join(folder,unique_file_name)
        return response.stream(open(path,'rb'),chunk_size=4096)
    else:
        grid="Besion ID du fichier"
    return dict(grid=grid)
