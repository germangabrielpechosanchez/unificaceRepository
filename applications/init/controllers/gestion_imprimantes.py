...
@auth.requires(auth.has_membership(role='admin')|auth.has_membership(role='gestion_imprimante_admin'))
...
def access_onvalidation(form):
    ...
    if form.vars.group_id not in groups_access:
        ...
        form.errors.group_id="Le groupe doit être dans cette liste: "+((str(rs).replace("auth_group.role","")).replace("gestion_imprimante",", gestion_imprimante"))
        form.errors= True
    return
