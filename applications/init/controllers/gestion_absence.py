...
def absences_menu():
    ...
    (T("Gestion des absences"), False, '#', [
        ...
        (T('Gestion des absences - win10'), False, URL('gestion_absence','index',args=["WIN10 ABSENCE",]),[]),
        (T('Gestion des absences - itsm'), False, URL('gestion_absence','index',args=["ITSM ABSENCE",]),[]),
        (T('Gestion des absences - aaesi'), False, URL('gestion_absence','index',args=["AAESI ABSENCE",]),[]),
        ]),
    (T('Gestion des absences - infra'), False, URL('gestion_absence','index',args=["INFRA ABSENCE",]),[]),
    (T('Gestion des absences - ops'), False, URL('gestion_absence','index',args=["OPERA ABSENCE",]),[]),
    (T('Gestion des absences - win10'), False, URL('gestion_absence','index',args=["WIN10 ABSENCE",]),[]),
...
def index():  
    ...
    if (this_object_group=="INFRA ABSENCE"):
        has_acces=True
    elif (this_object_group=="ITSM ABSENCE"):
        has_acces=True
    elif (this_object_group=="AAESI ABSENCE"):
        has_acces=True
    elif (this_object_group=="WIN10 ABSENCE")&(auth.has_membership(role='gestion_absence')|auth.has_membership(role='gestion_absence_read')):
        has_acces=True
    ...
    if has_acces:
        ...
        object_group_ids={"INFRA ABSENCE":1,"OPERA ABSENCE":2,"ITSM ABSENCE":4,"AAESI ABSENCE":3,"WIN10 ABSENCE":5,}
        ...
        if ('resource_view' in request.vars):
            resource_view_checked=True
        else:
            resource_view_checked=False
        ...
        form=FORM(
            TABLE(
                ...
                TD(LABEL("PAR RESSOURE ",INPUT(_type='checkbox',_class='my_label',_name='resource_view',value=resource_view_checked))),#all_view_checked
        ...
        if form.accepts(request.vars,session):
            ...
            if ('all_view' in request.vars):
                ...
                #msg+=str(object_list)
            if ('resource_view' in request.vars):
                qry_view=(qdb.rs_object==int(request.vars.object))&(qdb.resources==request.vars.resource)
            else:
                        ...
                        if (day<1)|(day>ldaym):
                            ...
                            if dic.has_key(this_day):
                                ...
                                for m in range(lend):
                                    ...
                                    style_bt='color: '+str(ft_c1)+';text-align: left;background-color:'+str(bt_c1)+';border-color: white;height:'+str(bt_h)+'em;'#;width:30em
                                    style_bt='color: '+str(ft_c1)+';text-align: left;background-color:'+str(bt_c1)+';border-color: white;'
                                    #style_td='background-color:'+str(td_c)+';height:'+str(bt_h)+'em;font-size:'+str(2)+'%;'
                                    style_td='background-color:'+str(td_c)+';font-size:'+str(2)+'%;'
    ...
    """
    rs=db((db.rs_type.id>=6)&(db.rs_type.id<=12)).select()
    for r in rs:
        db.rs_type.insert(title=r.title,description=r.description,color=r.color,object_group="AAESI ABSENCE")
    response.flash=rs
    """
...
def search():
    ...
    qry=db.reservation
    orderby=db.reservation.time_begin
    grid = SQLFORM.grid(qry,orderby=~orderby,maxtextlength=70,deletable=False,editable=False,create=False)
...
def update_resources_availability(this_resource,this_day):
    ...
    if (this_resource<>48)&(this_resource<>52): #48=DG;52=MGR
        ...
        excluded_teletravail=(~db.reservation.rs_type.belongs(11,18))
        qry=(db.reservation.resources==this_resource)&(db.reservation.time_begin>=this_begin)&(db.reservation.time_end<=this_end)&(db.reservation.replace_id==0)&(excluded_teletravail)
...
def update_capacity_availability(this_resource,this_day):
    #no update for Stephane et Marie-Claude
    if (this_resource<>1152)&(this_resource<>52): #52=MGR;1152=JOSE CORDEIRO
        availability=2100 #35*60
        ...
        if rs:
            ...
            this_absence=0
        this_availability=availability-this_absence
        if this_availability<0:
            this_availability=0
        ...
        #if this_absence:
        if rs:
            db(db.capacity_availability.id==rs[0].id).update(bank=this_availability)
        else:
            db.capacity_availability.insert(
                resources=this_resource,
                time_begin=this_begin,
                time_end=this_end,
                bank=this_availability
                )
    else:
        this_begin=this_day #just for return no error
    return (this_resource,this_begin)
