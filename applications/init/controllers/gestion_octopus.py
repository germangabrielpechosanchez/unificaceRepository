...
@auth.requires(auth.has_membership(role='admin')|auth.has_membership(role='gestion_octopus')|auth.has_membership(role='gestion_octopus_edit')|auth.has_membership(role='gestion_octopus_edit_add')|auth.has_membership(role='gestion_alerts_monitoring'))
def importation():
    response.menu+=[
        (T('Importation'), False, URL( f='importation'),[]),
        ]
    ...
    fs=db((db.oct_importation_files.imported==False)&(db.oct_importation_files.user_id==auth_id)).select(db.oct_importation_files.ALL)
    for f in fs:
    ...
    # executed by auto request
    if request.args(0)=='octopus_sccm_update_by_request':
        redirect(URL('octopus_sccm_update_by_request', args=['octopus_sccm_update_by_request_done',]))  
    # original
    if fs:
...
@auth.requires_login()
@request.restful()
def octopus_sccm_update_by_request():
    def GET(input):
        output="have no access"
        output=str(auth.has_membership(role='gestion_alerts_monitoring'))
        start=now
        if (auth.has_membership(role='gestion_alerts_monitoring'))&(input=="octopus_sccm_update_by_request"):
            #
            file_name=r'\\s01vwpr00021.cemtl.rtss.qc.ca\PlageMaintenance\PlageMaintenance.csv'
            this_file=open(file_name, 'r')
            auth_id=session.auth.user.id
            author=db(db.auth_user.id==auth_id).select(db.auth_user.username).first().username
            #.decode('utf8').encode('ISO-8859-1')#file tranformation og codage
            db.oct_importation_files.insert(user_id=auth_id,team=1,types='CI',dates=now,description="Géneré par le module de gestion des importation vers octopus (sccm plage de maintenance des serveurs ) :"+author,imported=False,files=db.oct_importation_files.files.store(this_file,file_name),file_data=this_file.read().decode('utf8').encode('ISO-8859-1'))
            this_file.close()
            redirect(URL('importation', args=[request.args(0),])) 
            output=str(datetime.datetime.now()-start)
            session.msg=output
        elif (auth.has_membership(role='gestion_alerts_monitoring'))&(input=="octopus_sccm_update_by_request_done"):
            output="done"
        return output
        session.forget()
    return locals()