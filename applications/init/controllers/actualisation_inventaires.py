...
def cis_listing():
    ...
    bad_char=" "
    good_char=" "
    rs=db(qry).select(db.act_cis.id,db.act_cis.description)
    for r in rs:
        id=db(db.act_cis.id==r.id).update(description=r.description.replace(bad_char,good_char))
...
def all_items():
    ...
    # bad_char=" "
    # good_char=" "
    # rs=db(db.act_cis.description>6666).select(db.act_cis.id,db.act_cis.description)
    # for r in rs:
    #     id=db(db.act_cis.id==r.id).update(description=r.description.replace(bad_char,good_char))
