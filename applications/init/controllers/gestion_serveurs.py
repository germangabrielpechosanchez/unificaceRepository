...
if auth.is_logged_in():
    if (auth.has_membership(role='admin')|auth.has_membership(role='gestion_serveurs')):
        ...
        searchable=True
        details=True
    elif (auth.has_membership(role='gestion_serveurs_edit_add')):
        create=True
        deletable=False
        editable=True
        user_signature=True
...
def inventory_no_ping_no_172():
    ...
    grid = SQLFORM.grid(query,user_signature=False,maxtextlength=50,deletable=deletable,editable=editable,create=create,searchable=searchable,details=details,buttons_placement = 'left')
...
def tables():
    ...
    deletable=deletable,editable=editable,create=create,user_signature=user_signature,searchable=searchable,details=details,buttons_placement = 'left')
...
def inventory_update():#inventory_update_from_ads_vcenters
    ...
    rs=db((db.ads_computers.OperatingSystem.like("%server%"))&(db.ads_computers.Enabled == "True")&(db.ads_computers.LastLogonDate > "2018-06-15")).select()
