...
def exportation():       
    ...
    db(db.ci_payment.exported==datetime.date(3000,1,1)).update(exported=today,to_update=True)
    #db((db.ci_payment.exported==datetime.date(3000,1,1))&(db.ci_payment.account<>9679).update(to_update=True)
...
def exportation_octopus():
    ...
    rs=db((db.ci_payment.exported<>datetime.date(3000,1,1))&(db.ci_payment.to_update==True)&(db.ci_payment.account==db.account_department.id)).select(db.ci_payment.name,db.account_department.account,db.ci_payment.exported)
    if rs:
        import subprocess     
        ...
        db((db.ci_payment.exported<>datetime.date(3000,1,1))&(db.ci_payment.to_update==True)).update(to_update=False) #mark as already syned
        ...
        r=db(db.ci_files.dates==today).select()
        if r:
            db(db.ci_files.dates==today).update(files_octopus=db.ci_files.files_octopus.store(this_file,file_name),file_data_octopus=this_file.read())
        else:
            db.ci_files.insert(user_id=session.auth.user.id,dates=today,description="retrofacturation",files_octopus=db.ci_files.files_octopus.store(this_file,file_name),file_data_octopus=this_file.read())
        this_file.close()
        #delete the file
        #os.remove(file_name)
...
def log_file():
    ...
    path=r'C:\Users\admphav\AppData\Local\Octopus_cemtl'
    file_name= path+r'\DataImporter.log'
    ...
    grid=grid.replace("gestion_retrofacturation","")
