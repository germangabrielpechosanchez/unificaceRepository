...
response.menu = [
    ...
    (T('Gestion Proxy'), False, URL('gestion_proxy', 'index'), []),
    #(T('Octopus - Paramètres'), False, URL('default', 'index'), []),


]

@auth.requires(auth.has_membership(role='admin')|auth.has_membership(role='gestion_proxy'))
def index():
    import subprocess 
    ...
    form = SQLFORM.factory(
        ...
        #Field('nom',label="Selectionez un profil",default='',writable=False),#,writable=False
        #Field('nom',label='Nom',default=this_user[0].last_name),#,writable=False
        #Field('profil_internet',label="Profil d'internet",default=profile_value,requires=IS_IN_SET(groups.keys())),#requires=IS_NOT_EMPTY()#default=groups.keys()[0]
    ...
    if form.process().accepted:
        ...
        #arg3=groups[profil]
        script=r'C:\Users\admphav\unificatex\web2py\applications\init\scripts\gestion_proxy\gestion_proxy_bta.ps1'#gestion_proxy.ps1
