...
def invoice_import():
    grid =""
    #fs=db(db.invoice_file.imported==False).select(db.invoice_file.id,db.invoice_file.file_data)
    fs=db(db.invoice_file.imported==False).select(db.invoice_file.id,db.invoice_file.file_data,orderby=db.invoice_file.dates)
    #db(db.invoices.id>0).delete()
    #fs=db(db.invoice_file.id>0).select(db.invoice_file.file_data)
    msg=""
    i=0
        ...
        if can_import:
            ...
            for l in ls:
                if l<>"":
                    ...
                    .select(db.client_departments.client_name,db.account_department.id,db.account_department.account,orderby=db.client_departments.id)
                    if ad: #client User_Number exist in table client_department
                        if ad[0].account_department.id==9679:
                            found_account=db(db.account_department.account==Sub_Level_B_Name).select(db.account_department.id,db.account_department.account)
                            if found_account: # account=code bugetaire exist
                                
                                db(db.client_departments.client_number==User_Number).update(account=found_account[0].id,client_name=User_Name)
                                Sub_Level_B_Name=found_account[0].account
                            else: #account=code bugetaire not exist
                                
                                Sub_Level_B_Name=ad[0].account_department.account
                                User_Name=ad[0].client_departments.client_name                                
                        else:
                            #msg="ici"    
                            Sub_Level_B_Name=ad[0].account_department.account
                            User_Name=ad[0].client_departments.client_name
                    else: #client not exist in table client_department
                        i+=1
                        ...
                        found_account=db(db.account_department.account==Sub_Level_B_Name).select(db.account_department.id)
                        if found_account: # account=code bugetaire exist
                            db.client_departments.insert( account=found_account[0].id,client_number=User_Number,client_name=User_Name)
                        else: #account=code bugetaire not exist
                            db.client_departments.insert( account=9679,client_number=User_Number,client_name=User_Name)
                            Sub_Level_B_Name="xxx "+Sub_Level_B_Name
...
def export_csvs_zip():
    ...
    file_names=()
    if rs:
        #file_name='facture_telus_'+str(now).replace('.','').replace(':','').replace(' ','')+'_'+str(i)+'.csv'
        this_date_facture=rs[0].Date_facture
        no_facture=rs[0].No_facture
        #file_name='facture_telus_'+this_date_facture+'_'+str(now).replace('.','').replace(':','').replace(' ','')+'_'+str(i)+'.csv'
    ...
    #file_name='facture_telus_'+str(now).replace('.','').replace(':','').replace(' ','')+'.zip'
    #file_name='facture_telus_'+this_date_facture+'_'+str(now).replace('.','').replace(':','').replace(' ','')+'.zip'
...
def tables():
    menu_url=[]
    tables={"Départements - Code budgétaire":"account_department","Clients - Numero de téléphone":"client_departments","Gestionaires aux départements":"managers_to_account","Gestionaires":"managers","Facture - détails":"invoices"}
