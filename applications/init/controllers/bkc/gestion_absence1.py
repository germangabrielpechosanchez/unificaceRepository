...
def index():
    ...
    tbl_object=db((db.rs_object.object_group=="INFRA ABSENCE")).select(db.rs_object.id,db.rs_object.title)
    object_items=[]
    object_list=()
    for i in tbl_object:
        ...
        object_list+=(i.id,)
    #rs_type
    tbl_type=db((db.rs_type.object_group=="INFRA ABSENCE")).select(db.rs_type.id,db.rs_type.title,db.rs_type.color)
    type_items=[]
    color_items={}
    for i in tbl_type:
    ...
    if form.accepts(request.vars,session):
        ...
        if request.vars.has_key('actualiser'):
            ...
            if (len(rs)==1):
                if (rs[0].time_begin>=now)&(begin>=now)&(begin<end)&(rs[0].replace_id==0)|auth.has_membership(role='gestion_absence'):
            ...
            if (len(rs)==1):
                if (rs[0].time_begin>=now)&(rs[0].replace_id==0)|auth.has_membership(role='gestion_absence'):
                    if (rs[0].edit_by==auth.user_id): #auth.has_membership(role='users_rs')|(rs[0].edit_by==auth.user_id):
                        this_record=this_id
                        rs=db(qdb.id==this_id).update(\
