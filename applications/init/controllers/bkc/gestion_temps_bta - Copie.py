...
def index():
    ...
    tbl_object=db((db.rs_object.object_group=="INFRA")).select(db.rs_object.id,db.rs_object.title)
    object_items=[]
    object_list=()
    for i in tbl_object:
        ...
        object_list+=(i.id,)
    #rs_type
    tbl_type=db((db.rs_type.object_group=="INFRA")).select(db.rs_type.id,db.rs_type.title,db.rs_type.color)
    type_items=[]
    color_items={}
    for i in tbl_type:
...
def tables():
    menu_url=[]
    tables=["rs_object","rs_type","rs_object_type""reservation",]
