...
def inventory_input():
    ...
    if session.inventory_input_mode=="manual":
        ...
        fields=[
            ...
            Field('oct_cc',label=T('Centre de coûts'),default=oct_cc,comment="Pour 'Compte Bilan' ou 'Maintien des actifs (MAI)', le centre de Coûts sera assigné automatiquement. Pour 'Mobilité' il faut mettre le 7340205"),
    ...
    if form.accepts(request.vars,session,keepvalues=form_keepvalues):
        if request.vars.has_key("suivant")|request.vars.has_key("actualiser"):
            ...
            try:
                ...
                nom=db(db.inv_cis.TypeCi.like(session.type_ci,case_sensitive=False)).select(max).first()[max]
                nom=nom.upper()
                ...
                db.inv_cis.insert(
                    ...
                    Total=session.qty,
                    NumeroBonCommande=session.bill,
                ...
                session.msg="Il faut ajouter ce type de ci dans les tables de configuration (TYPE de CI) et dernière numéro de ce type de ci dans historique (numéro doit dans cette form ABCD12345) OU il y a double de série de même type CI."
...
def inventory_import():
    ...
    if rs:
        ...
        for r in rs:
            ...
            inventoryNumber=""
            if  TypeCi=="Téléphone mobile":
                Category="Cellulaire"
            elif TypeCi=="Dispositif internet mobile":
                Category="Clé USB internet"
            elif TypeCi=="Ligne mobile":
                Category="Voix"
            ...
            if r.SourceDeFinancement in ("Compte Bilan","Maintien des actifs (MAI)","Projet"):
                reserve_pour_inventare=r.SourceDeFinancement
            else:
