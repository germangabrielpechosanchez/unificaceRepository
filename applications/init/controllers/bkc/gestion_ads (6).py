...
def creation_comptes():
    ...
    form = SQLFORM.factory(
        ...
        Field('users',type='text',label=T('Liste des utilisateurs à créer'),length=20000,requires=IS_NOT_EMPTY(),comment=T("FORMAT : Prénom nom;titre d'emploi;Groupe AD (On peut avoir plus que 500 lignes-utilisateurs)"),represent=lambda text, row: XML(text.replace('\n', '<br />'),sanitize=True, permitted_tags=['br/'])),
    ...
    if form.accepts(request.vars,session,keepvalues=True):#pour eviter dernier ligne vide il viens de Powershell
        ...
        arg1=arg1=request.vars.domain+"***"+request.vars.users #adaptation to send args " " and ";" to Powershell
