        ...
        for i in range(1,count_folders):
            ...
            if path_current not in root:
                ...
                if (level==solution_level)&(r.types=="Folder"):
                    grid+='<div class="dropdown"><label class="dropbtn" for="'+id_folder+'">'+folders_display+'</label><div class="dropdown-content"><a href="https://unificace2.cemtl.rtss.qc.ca/init/gestion_vms_bta/add_solution/'+r.folder_id+'">Ajouter solution</a></div></div><input type="checkbox" '+checked_enabled+' id="'+id_folder+'" />'
                elif (level==solution_level+1)&(r.types=="Folder"):
                    grid+='<div class="dropdown"><label class="dropbtn1" for="'+id_folder+'">'+folders_display+'</label><div class="dropdown-content"><a href="https://unificace2.cemtl.rtss.qc.ca/init/gestion_vms_bta/get_vm_data/'+r.folder_id+'">Créer serveur</a></div></div><input type="checkbox" '+checked_enabled+' id="'+id_folder+'" />'
                elif r.types=="Folder":
                    ...
                    grid+='<li><div class="dropdown"><div class="dropbtn2">'+folders_display+'</div><div class="dropdown-content"><a href="https://unificace2.cemtl.rtss.qc.ca/init/gestion_vms_bta/modify_vm/'+r.folder_id+'">Modifier ce VM</a></div></div></li>'
...
def get_vm_data():
    ...
    url="http://unificace2.cemtl.rtss.qc.ca:8080/gestion_vms_api.ps1?command=get_vm_data&vcenter="+vcenter+"&folder="+folder
    ...
    if form.accepts(request.vars,session,keepvalues=True):
        url="http://unificace2.cemtl.rtss.qc.ca:8080/gestion_vms_api.ps1?command=create_vm_server&vcenter="+vcenter+"&datacenter="+datacenter_id+"&folder="+folder+"&template="+request.vars.template+"&vlan="+request.vars.vlan+"&domain="+domain+"&environment="+environment+"&solution="+solution
        outs=requests.get(url,stream=True).content
        #outs="S03VLDV00001 Network adapter 1 ;S03VLDV00001;Folder-group-v182121;VirtualMachine-vm-182595; S03VLDV00001"
        #";$VMname;$folder;$vm_id;"
        #"""
        if outs:
            f=outs.split(";")
        ...
        response.flash =  "Ne peut pas créer ce server"
        #"""
    #response.flash = outs
    #return dict(form =form,grid= url)
    return dict(form =form,grid= outs)
...
def add_solution():
    ...
    if form.accepts(request.vars,session,keepvalues=True):
        ...
        url="http://unificace2.cemtl.rtss.qc.ca:8080/gestion_vms_api.ps1?command=add_solution&vcenter="+vcenter+"&folder="+folder+"&solution="+solution
        outs=requests.get(url,stream=True).content
        if outs:
            #";$folder;$new_folder;"
            f=outs.split(";")
