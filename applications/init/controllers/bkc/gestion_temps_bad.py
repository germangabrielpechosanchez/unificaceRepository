...
@auth.requires(auth.has_membership(role='admin')|auth.has_membership(role='gestion_temps'))

def index():
    response.menu+=[
        (T('Calendrier'), False, URL( f='index'),[]),
        ]
    tbl_object=db((db.rs_object.is_active==True)).\
                   select(db.rs_object.id,db.rs_object.title)
    object_items=[]
    for i in tbl_object:object_items+=[(i.title.upper(),i.id),]
    #rs_type
    tbl_type=db((db.rs_type.id>0)).\
                   select(db.rs_type.id,db.rs_type.title,db.rs_type.color)
    type_items=[]
    color_items=[]
    for i in tbl_type:
        type_items+=[(i.id,i.title.upper()),]
        color_items+=[(i.id,i.color.upper()),]
    ...
    form=FORM(
        TABLE(
            TR(
            #TD(XML("<b><font color='blue'>"+"TYPE DE CALENDRIER: "+"</b></font>"),SELECT(*[OPTION(v,_value=k) for (v,k) in object_items],_name='object',_value=request.vars.object)),
            TD(LABEL("TYPE DE CALENDRIER ",_class='label'),SELECT(*[OPTION(v,_value=k) for (v,k) in object_items],_class='class_input',_name='object',_value=request.vars.object)),
            ...
            TD(LABEL("VUE PAR ",_class='label'),SELECT(*[OPTION(v,_value=k) for (k,v) in view.items()],_class='class_input',_name='view_by',_value=request.vars.view_by)),
            ),
            TR(''),
            TR(''),
            TR(
    ...
    if (auth.has_membership(role='admin')|auth.has_membership(role='gestion_temps')):
        action_button='submit'
    else:
    ...
    if form.accepts(request.vars,session):
        ...
        if not request.vars.object: request.vars.object='0'
        ...
        if request.vars.has_key('aujourd\'hui'):
            ...
            if (len(rs)==1):
                if (rs[0].time_begin>=now)&(begin>=now)&(begin<end)&(rs[0].replace_id==0):
            ...
            if (len(rs)==1):
                if (rs[0].time_begin>=now)&(rs[0].replace_id==0):
                    if (rs[0].edit_by==auth.user_id): #auth.has_membership(role='users_rs')|(rs[0].edit_by==auth.user_id):
                        this_record=this_id
                        rs=db(qdb.id==this_id).update(\
        ...
        tdb_set=tdb_set&((qdb.rs_object==int(request.vars.object))&\
                 (qdb.rs_object==db.rs_object.id)&\
                 (qdb.edit_by==db.auth_user.id))
        ...
        for rs in rc:
            ...
            if not strike_record:
                ...
                if red_rs>1:
                    ...
                    red_record=False
            #end red record
            j=(rs.reservation.id,rs.reservation.time_begin,rs.reservation.time_end,rs.reservation.title,rs.auth_user.last_name,red_record,strike_record)
            if dic.has_key(i):
                dic[i]+=(j,)
            else:
                dic[i]=(j,)
        ...
        #TD(LABEL("VUE PAR ",_class='label'),SELECT(*[OPTION(v,_value=k) for (k,v) in view.items()],_class='class_input',_name='view_by',_value=request.vars.view_by)),
        ##form[0][0].insert(6,TD(T('Begin:'),INPUT(_type='text',_class='date',_name='begin',_id='begin',value=value_begin,_size=10,requires=IS_DATE())))
        form[0][0].insert(1,TD(LABEL("VUE PAR ",_class='label'),INPUT(_type='text',_class='date class_input',_name='date class_input',_id='date',value=value_date,_size=10,requires=IS_DATE())))#2 class _class="date input"
                    ...
                    for j in range(len(rcs.colnames)):
                        if i==1:
                            ...
                            if j==1:
                                ...
                                #TD(LABEL("VUE PAR ",_class='label'),SELECT(*[OPTION(v,_value=k) for (k,v) in view.items()],_class='class_input',_name='view_by',_value=request.vars.view_by)),
                                form_details[i].insert(j,TD(TEXTAREA(_type='text',_class='class_input',_name='title',value=value_title,_rows=1,_cols=30)))
                                #form_details[i].insert(j,TD(TEXTAREA(_type='hidden',_readonly='on',_name='title',value=value_title,_rows=1,_cols=20)))
                            elif j==3:
                                form_details[i].insert(j,TD(TEXTAREA(_type='text',_class='class_input',_name='description',value=value_description,_rows=2,_cols=50)))
                            elif j==4:
                                form_details[i].insert(j,TABLE(\
                                    TR(TABLE(TR(\
                                        TD(SELECT(*[OPTION(v,_value=k) for (k,v) in \
                                                    [(6, '06'), (7, '07'), (8, '08'), (9, '09'),(10, '10'), (11, '11'), (12, '12'), (13, '13'), (14, '14'), (15, '15'), (16, '16'), (17, '17'), (18, '18'), (19, '19'), (20, '20')]\
        ...
        form_details.insert(0,TR(XML(stt),_style='text-align:center;'))#TH ; MARCH / 2017
        ...
        for i in range(7):
            ...
            for j in range(7):
                if i==0:
                    ...
                    if (day<1)|(day>ldaym):
                        ...
                        if dic.has_key(this_day):
                            ...
                            for m in range(lend):
                                ...
                                valdis=str(dic[this_day][m][1].hour)+\
                                    ":"+str(dic[this_day][m][1].minute)+\
                                    ".."+str(dic[this_day][m][3])+\
                                    "\n"+str(dic[this_day][m][2].hour)+\
                                    ":"+str(dic[this_day][m][2].minute)+\
                                    ".."+str(dic[this_day][m][4])+\
                                    ".    #"+str(dic[this_day][m][0])
                    ...
                    form_details[i][j].append(DIV(INPUT(_class='button_calendar',_type='submit',\
                    _value=display,_id=display,_name=display,_style=style_bt)))
...
def tables():
    menu_url=[]
    tables=["rs_object","rs_type","reservation",]
