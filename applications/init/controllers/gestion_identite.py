...
response.menu = [
    ...
    (T('Gestion dentité'), False, URL('gestion_identite', 'index'), []),
    #(T('Octopus - Paramètres'), False, URL('default', 'index'), []),


]

@auth.requires(auth.has_membership(role='admin')|auth.has_membership(role='gestion_identite'))
def index():
    import subprocess 
    ...
    return dict(grid='',form=form,request=request)
